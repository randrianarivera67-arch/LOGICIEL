...
function openPay(item){curItem=item;curMethod=null;closePay();ov=document.createElement('div');ov.className='modal-ov';
var mh='';for(var i=0;i<METHODS.length;i++){var m=METHODS[i];mh+='<div class="pm-m" data-m="'+m.id+'"><img src="'+m.img+'" alt="" style="width:52px;height:52px;border-radius:12px;object-fit:cover" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'grid\'"><div class="pm-fb" style="background:'+m.fb+';display:none">'+m.name+'</div><span>'+m.name+'</span></div>'}
ov.innerHTML='<div class="modal"><h3 style="font-family:Sora">💳 Paiement — '+escS(item.name)+'</h3><p style="color:var(--amber);font-family:Sora;font-weight:800;font-size:1.2rem;margin:6px 0">'+escS(item.price||'—')+'</p><p style="color:var(--muted);font-size:.84rem">Safidio ny fomba fandoavana :</p><div class="pm-methods">'+mh+'</div><div id="pmStep2"></div><div style="display:flex;gap:10px;margin-top:14px"><button class="btn ghost" style="flex:1" onclick="closePay()">Annuler</button></div></div>';
document.body.appendChild(ov);
var els=ov.querySelectorAll('[data-m]');for(var j=0;j<els.length;j++){(function(el){el.onclick=function(){var all=ov.querySelectorAll('.pm-m');for(var k=0;k<all.length;k++){all[k].classList.remove('sel')}el.classList.add('sel');curMethod=el.getAttribute('data-m');showStep2()}})(els[j])}}
function showStep2(){var m=null;for(var i=0;i<METHODS.length;i++){if(METHODS[i].id===curMethod){m=METHODS[i];break}}var num=SETTINGS[m.key]||'—';var isP=curMethod==='paypal';
g('pmStep2').innerHTML='<div class="pm-box">'+(isP?'Envoyez le montant à l\'adresse PayPal :':'Envoyez <b>'+escS(curItem.price||'')+'</b> au numéro :')+'<br><span class="pm-num">'+escS(num)+'</span><br>Puis entrez ci-dessous la référence de la transaction.</div><label style="display:block;margin:10px 0 0;font-size:.82rem;color:var(--muted)">Référence transaction<input id="pmRef" placeholder="Ex : 123456789"></label><label style="display:block;margin:8px 0 0;font-size:.82rem;color:var(--muted)">Votre numéro (nandefasana)<input id="pmFrom" placeholder="Ex : 034 …"></label><button class="btn primary big" style="margin-top:12px" onclick="confirmPay()">✅ J\'ai payé — Confirmer</button>'}
