...
function rows(items){return items.map(function(it){var thumb=it.image?'<img src="'+esc(it.image)+'" alt="">':esc(initials(it.name));return '<div class="a-row"><div class="a-thumb cg'+(hashN(it.name)%5)+'">'+thumb+'</div><div class="a-info"><b>'+esc(it.name)+'</b><span>'+esc(it.type||'Logiciel')+' • '+esc(it.os||'—')+' • '+esc(it.category||'Autre')+'</span></div><span class="a-price">'+esc(it.price||'—')+'</span><div class="a-actions"><button data-edit="'+esc(it.id)+'" type="button"><svg class="ni ni-sm"><use href="#i-edit"></use></svg> Modifier</button><button data-del="'+esc(it.id)+'" class="del" type="button"><svg class="ni ni-sm" style="color:#fca5a5;filter:drop-shadow(0 0 4px rgba(244,63,94,.7))"><use href="#i-trash"></use></svg> Suppr.</button></div></div>'}).join('')||'<p style="color:var(--muted)">Aucun produit.</p>'}
function applyCatFilter(){var cat=window.__admCat||'Tous';var q=(g('adminSearch')?g('adminSearch').value:'').toLowerCase();var items=(state.list||[]).filter(function(it){if(cat!=='Tous'&&(it.category||'Autre')!==cat)return false;if(q&&(it.name||'').toLowerCase().indexOf(q)===-1&&(it.category||'').toLowerCase().indexOf(q)===-1)return false;return true});g('listCount').textContent=items.length;g('adminList').innerHTML=rows(items)}
...
var html='<div class="panel dash-c"><b>'+L.length+'</b><span>Produits</span></div><div class="panel dash-c"><b>'+nbLog+'</b><span>Logiciels</span></div><div class="panel dash-c"><b>'+(L.length-nbLog)+'</b><span>Applications</span></div><div class="panel dash-c"><b>'+Object.keys(cats).length+'</b><span>Catégories</span></div><div class="panel dash-c"><b>'+totDl.toLocaleString()+'</b><span>Téléchargements</span></div><div class="panel" style="grid-column:1/-1"><h3>🏆 Top 5 téléchargements</h3>'+top.map(function(i){return '<div style="display:flex;justify-content:space-between;gap:10px;padding:8px 0;border-bottom:1px dashed var(--border);font-size:.88rem"><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+esc(i.name)+'</span><b style="color:var(--amber);flex:none">'+(i.dl||0).toLocaleString()+'</b></div>'}).join('')+'</div><div class="panel" style="grid-column:1/-1"><h3>📂 Répartition par catégorie</h3>'+Object.keys(cats).map(function(c){return '<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px dashed var(--border);font-size:.88rem"><span>'+esc(c)+'</span><b>'+cats[c]+'</b></div>'}).join('')+'</div>';
...
var h='<a class="btn ghost" href="#/">🏠 Accueil du site</a>';
for(var i=0;i<cats.length;i++){(function(c){h+='<button class="btn ghost" onclick="pickCat(\''+c.replace(/'/g,"\\'")+'\')">📂 '+esc(c)+'</button>'})(cats[i])}
h+='<button class="btn ghost" onclick="openDrawer()">🛒 Panier</button><button class="btn ghost" onclick="toggleTheme()">🌓 Clair / Sombre</button><button class="btn ghost" onclick="doLogout()">🚪 Déconnexion</button>';
...
sNav.innerHTML='<div class="panel"><h3>🔗 Navigation rapide</h3><div id="navLinks" style="display:flex;flex-direction:column;gap:8px"></div></div>';
